...
static void StartLoadCover(int set_id, const std::string& url) {
    if (url.empty() || set_id == s_cover_set_id || set_id == s_loading_cover_id) return;
    ...
    std::string cmd =
        "curl -s -L --max-time 10 -o \"" + s_cover_tmp + "\" \"" + url + "\" 2>&1";
    s_cover_pipe = popen(cmd.c_str(), "r");
    if (!s_cover_pipe) { s_loading_cover_id = -1; }
}

static void PollCoverLoad() {
    if (!s_cover_pipe) return;
    char buf[256];
    while (fgets(buf, sizeof(buf), s_cover_pipe)) {} // drain
    ...
    Texture2D tex = LoadTexture(s_cover_tmp.c_str());
    if (tex.id > 0) {
...
static float FindJsonFloat(const std::string& json, const std::string& key) {
    ...
    if (json[pos] == '"') { ++pos; size_t e=json.find('"',pos); return e==std::string::npos?0.0f:(float)atof(json.substr(pos,e-pos).c_str()); }
...
static std::string FindJsonValue(const std::string& json, const std::string& key) {
    ...
    if (open=='{' || open=='[') {
        char close = (open=='{') ? '}' : ']';
        int depth=0; size_t start=pos; bool in_str=false;
        while (pos<json.size()) {
            char c=json[pos];
            if (in_str) { if(c=='\\')++pos; else if(c=='"')in_str=false; }
            else { if(c=='"')in_str=true; else if(c==open)++depth; else if(c==close){--depth;if(depth==0){++pos;return json.substr(start,pos-start);}} }
            ++pos;
        }
        return "";
    }
    if (open=='"') {
        ++pos; std::string r;
        while(pos<json.size()&&json[pos]!='"'){if(json[pos]=='\\'&&pos+1<json.size())++pos;r+=json[pos++];}
        return r;
    }
    size_t start=pos;
...
static void IterJsonArray(const std::string& arr, const std::function<void(const std::string&)>& cb) {
    ...
    size_t pos=1;
    while(pos<arr.size()) {
        while(pos<arr.size()&&(arr[pos]==' '||arr[pos]=='\t'||arr[pos]=='\r'||arr[pos]=='\n'||arr[pos]==','))++pos;
        if(pos>=arr.size()||arr[pos]==']') break;
        if(arr[pos]=='{') {
            int depth=0; size_t start=pos; bool in_str=false;
            while(pos<arr.size()){char c=arr[pos];if(in_str){if(c=='\\')++pos;else if(c=='"')in_str=false;}else{if(c=='"')in_str=true;else if(c=='{')++depth;else if(c=='}'){--depth;if(depth==0){++pos;break;}}}++pos;}
...
static std::vector<BeatmapSet> ParseSearchResponse(const std::string& json, std::string& out_cursor) {
    ...
    IterJsonArray(sets_arr, [&](const std::string& obj) {
        ...
        if (!covers.empty()) {
            ...
            if (bs.cover_url.empty()) bs.cover_url = FindJsonString(covers, "card");
        }

        std::string bmaps_arr = FindJsonValue(obj, "beatmaps");
        if (!bmaps_arr.empty()) {
            IterJsonArray(bmaps_arr, [&](const std::string& bobj) {
                if (FindJsonString(bobj,"mode")!="mania" && FindJsonInt(bobj,"mode_int")!=3) return;
                BeatmapDiff d;
        ...
        std::sort(bs.diffs.begin(), bs.diffs.end(),
                  [](const BeatmapDiff& a, const BeatmapDiff& b){ return a.star_rating < b.star_rating; });
        if (bs.id != 0) results.push_back(std::move(bs));
    });
    return results;
}
...
static Color StatusColor(const std::string& s) {
    if (s=="ranked")    return UI_ACCENT;
    if (s=="loved")     return Color{255,102,170,255};
    if (s=="qualified") return Color{100,180,255,255};
    if (s=="approved")  return Color{100,220,130,255};
...
static std::string TruncateText(Font font, const std::string& text,
                                 float max_w, float fs, float sp) {
    if (MeasureTextEx(font, text.c_str(), fs, sp).x <= max_w) return text;
    std::string t = text;
    while (!t.empty()) {
        t.pop_back();
        if (MeasureTextEx(font, (t+"...").c_str(), fs, sp).x <= max_w) return t+"...";
    }
    return "...";
}
...
static std::string BuildSearchUrl(const std::string& query, int sf, int /*kf*/,
    ...
    if (sf==1) url += "&s=ranked";
    else if (sf==2) url += "&s=loved";
    else if (sf==3) url += "&s=qualified";
    if (!cursor.empty()) url += "&cursor_string=" + UrlEncode(cursor);
    return url;
}
...
static void StartSearch(bool append = false) {
    ...
    search_accum.clear();
    if (!append) { results.clear(); cursor_string.clear(); scroll_y = scroll_target = 0.0f; selected_idx = -1; UnloadCover(); }
    ...
    if (!search_pipe) { search_state = SearchState::ERROR; error_msg = "Failed to launch curl."; return; }
...
static void DrainSearchPipe() {
    ...
    if (!feof(search_pipe)) return;

    pclose(search_pipe); search_pipe = nullptr;
    if (search_accum.empty()) { search_state=SearchState::ERROR; error_msg="Empty response."; return; }

    size_t brace = search_accum.find('{');
    if (brace==std::string::npos) { search_state=SearchState::ERROR; error_msg="Bad response: "+search_accum.substr(0,120); return; }
    ...
    if (keys_filter != 0) {
        ...
        for (auto& bs : new_results)
            bs.diffs.erase(std::remove_if(bs.diffs.begin(),bs.diffs.end(),
                [target_cs](const BeatmapDiff& d){ return fabsf(d.cs-target_cs)>0.5f; }),
                bs.diffs.end());
        new_results.erase(std::remove_if(new_results.begin(),new_results.end(),
            [](const BeatmapSet& bs){ return bs.diffs.empty(); }), new_results.end());
    }

    for (auto& bs : new_results) results.push_back(std::move(bs));
...
static bool DrawResultCard(int id, Rectangle bounds, const BeatmapSet& bs, bool selected) {
    ...
    float dw = bounds.width*(1-sc)*0.5f, dh = bounds.height*(1-sc)*0.5f;
    Rectangle r = {bounds.x+dw, bounds.y+dh, bounds.width-dw*2, bounds.height-dh*2};

    DrawRectRounded(r, 10.0f, selected ? UI_SURFACE2 : ColorLerp(UI_SURFACE,UI_SURFACE2,ws.hover.value));
    DrawRectRoundedBorder(r, 10.0f, 1.5f,
        selected ? ColorAlpha(UI_ACCENT,0.55f) : ColorLerp(UI_BORDER,ColorAlpha(UI_ACCENT,0.35f),ws.hover.value));
    ...
    if (selected && s_cover.id > 0 && s_cover_set_id == bs.id) {
        ...
        float ta = cover_w/cover_h, xa = (float)s_cover.width/(float)s_cover.height;
        if (xa > ta) { float nw=s_cover.height*ta; src.x=(s_cover.width-nw)*0.5f; src.width=nw; }
        else         { float nh=s_cover.width/ta;  src.y=(s_cover.height-nh)*0.5f; src.height=nh; }
        DrawTexturePro(s_cover, src, {cover_x,cover_y,cover_w,cover_h}, {0,0}, 0.0f, WHITE);
        DrawRectangleRoundedLinesEx({cover_x,cover_y,cover_w,cover_h},
            6.0f/fminf(cover_w,cover_h), 8, 1.0f, ColorAlpha(UI_ACCENT,0.3f));
    } else {
        Color col = ColorAlpha(StatusColor(bs.status), 0.25f+ws.hover.value*0.1f);
        DrawRectRounded({cover_x,cover_y,cover_w,cover_h}, 6.0f, col);
        DrawRectRoundedBorder({cover_x,cover_y,cover_w,cover_h}, 6.0f, 1.0f, ColorAlpha(StatusColor(bs.status),0.4f));
        if (!bs.title.empty()) {
    ...
    DrawTextEx(ui.font_heading, TruncateText(ui.font_heading,bs.title,avail,16.0f,0.3f).c_str(),
               {tx,r.y+10.0f}, 16.0f, 0.3f,
               selected ? UI_ACCENT : ColorLerp(UI_TEXT,UI_ACCENT,ws.hover.value*0.3f));
    DrawTextEx(ui.font_body, TruncateText(ui.font_body,bs.artist,avail,12.0f,0.3f).c_str(),
    ...
    for (auto& c:su) c=(char)toupper((unsigned char)c);
    Color sc_col = StatusColor(bs.status);
    float badge_w = MeasureTextEx(ui.font_body,su.c_str(),10.0f,1.0f).x+12.0f;
    ...
    DrawTextCentered(ui.font_body,su.c_str(),badge_x+badge_w*0.5f,r.y+11.0f,10.0f,1.0f,sc_col);

    char bpm_buf[32]; snprintf(bpm_buf,sizeof(bpm_buf),"%.0f BPM",bs.bpm);
    DrawTextRight(ui.font_body,bpm_buf,rx2,r.y+32.0f,12.0f,0.3f,UI_TEXT_MUTED);

    if (!bs.diffs.empty()) {
        char sb[32]; snprintf(sb,sizeof(sb),"%.2f*",bs.diffs.back().star_rating);
        DrawTextRight(ui.font_body,sb,rx2,r.y+48.0f,12.0f,0.3f,StarColor(bs.diffs.back().star_rating));
    }

    bool has_4k=false,has_7k=false;
    for (const auto& d:bs.diffs){if(fabsf(d.cs-4.0f)<0.5f)has_4k=true;if(fabsf(d.cs-7.0f)<0.5f)has_7k=true;}
    float kbx=rx2;
    auto DrawKeyBadge = [&](const char* label) {
        float kbw=28.0f; kbx-=kbw+2.0f;
        ...
        DrawTextCentered(ui.font_body,label,kbx+kbw*0.5f,r.y+65.0f,9.0f,0.5f,UI_ACCENT);
    };
    if (has_7k) DrawKeyBadge("7K");
    if (has_4k) DrawKeyBadge("4K");
...
static void DrawDetailPanel(float x, float y, float w, float h,
    ...
    if (s_cover.id > 0 && s_cover_set_id == bs.id) {
        ...
        DrawTexturePro(s_cover,src,{px,py,cvw,cvh},{0,0},0.0f,ColorAlpha(WHITE,0.85f));
        DrawRectangleGradientV((int)px,(int)(py+cvh*0.5f),(int)cvw,(int)(cvh*0.5f),
                               ColorAlpha(BLACK,0),ColorAlpha(BLACK,0.5f));
        DrawRectangleRoundedLinesEx({px,py,cvw,cvh},8.0f/fminf(cvw,cvh),8,1.5f,
        ...
        for (int i=0;i<10;i++) {
            ...
            DrawCircleV({px+cvw*0.5f+cosf(a)*16.0f,py+cvh*0.5f+sinf(a)*16.0f},
                        2.5f+((float)i/10)*2.0f, ColorAlpha(UI_ACCENT,(float)i/10));
        }
    } else {
        DrawRectRounded({px,py,cvw,cvh},8.0f,ColorAlpha(UI_ACCENT,0.12f));
        DrawRectRoundedBorder({px,py,cvw,cvh},8.0f,1.5f,ColorAlpha(UI_ACCENT,0.25f));
        if (!bs.title.empty()) {
            char letter[2]={(char)toupper((unsigned char)bs.title[0]),'\0'};
            DrawTextCentered(ui.font_heading,letter,px+cvw*0.5f,py+cvh*0.5f-24.0f,
    ...
    std::string su=bs.status; for(auto& c:su) c=(char)toupper((unsigned char)c);
    Color sc_col=StatusColor(bs.status);
    float badge_w=MeasureTextEx(ui.font_body,su.c_str(),10.0f,1.0f).x+12.0f;
    ...
    DrawTextCentered(ui.font_body,su.c_str(),badge_x+badge_w*0.5f,py,10.0f,1.0f,sc_col);
    py+=22.0f;

    DrawLineEx({px,py},{x+w-20.0f,py},1.0f,UI_BORDER); py+=10.0f;
    DrawTextEx(ui.font_body,"DIFFICULTIES",{px,py},11.0f,2.0f,UI_TEXT_MUTED); py+=18.0f;

    float dh2=28.0f, diff_area_h=160.0f;
    int max_diffs=(int)(diff_area_h/dh2);
    for (int i=0;i<(int)bs.diffs.size()&&i<max_diffs;i++) {
        float iy=py+i*dh2;
        bool row_hov=CheckCollisionPointRec(GetMousePosition(),{px,iy,cvw,dh2-2.0f});
        DrawRectRounded({px,iy,cvw,dh2-2.0f},4.0f,
            row_hov?ColorAlpha(UI_ACCENT,0.07f):ColorAlpha(UI_SURFACE2,0.5f));
        Color dc=StarColor(bs.diffs[i].star_rating);
        DrawCircleV({px+10.0f,iy+dh2*0.5f-1.0f},4.0f,dc);
        DrawTextEx(ui.font_body,
                   TruncateText(ui.font_body,bs.diffs[i].version,cvw-100.0f,12.0f,0.3f).c_str(),
                   {px+20.0f,iy+dh2*0.5f-7.0f},12.0f,0.3f,UI_TEXT);
        char sb[16]; snprintf(sb,sizeof(sb),"%.2f*",bs.diffs[i].star_rating);
        DrawTextRight(ui.font_body,sb,x+w-60.0f,iy+dh2*0.5f-7.0f,11.0f,0.3f,dc);
        char kb[8]; snprintf(kb,sizeof(kb),"%.0fK",bs.diffs[i].cs);
        DrawTextRight(ui.font_body,kb,x+w-20.0f,iy+dh2*0.5f-7.0f,11.0f,0.3f,UI_TEXT_MUTED);
    }
    if ((int)bs.diffs.size()>max_diffs) {
        char mb[32]; snprintf(mb,sizeof(mb),"+%d more...",(int)bs.diffs.size()-max_diffs);
        DrawTextEx(ui.font_body,mb,{px,py+max_diffs*dh2},11.0f,0.3f,ColorAlpha(UI_TEXT_MUTED,0.6f));
    }
    py+=diff_area_h+8.0f;

    DrawLineEx({px,py},{x+w-20.0f,py},1.0f,UI_BORDER); py+=12.0f;
    ...
    if (is_this_set && dl.state!=DownloadStatus::State::IDLE) {
        Color mc=(dl.state==DownloadStatus::State::ERROR)?UI_DANGER:
                 (dl.state==DownloadStatus::State::DONE)?UI_ACCENT:UI_TEXT_MUTED;
        DrawTextEx(ui.font_body,
        ...
        if (dl.state==DownloadStatus::State::DOWNLOADING||
            ...
            float t=(float)GetTime(); float seg=fmodf(t*0.6f,1.0f);
    ...
    if (UIButtonGhost(BTN_DETAIL_BACK,{px,btn_y,btn_w,36.0f},"< BACK",13.0f)) {
        view=BeatmapView::LIST; detail_slide.Set(0.0f); selected_idx=-1; UnloadCover();
    }

    bool busy = osu_downloader.Busy() && is_this_set;
    bool done = is_installed || (is_this_set && dl.state==DownloadStatus::State::DONE);

    if (busy) {
        if (UIButton(BTN_DETAIL_CANCEL,{px+btn_w+12.0f,btn_y,btn_w,36.0f},"CANCEL",13.0f))
            osu_downloader.Cancel();
    } else if (done) {
        DrawRectRounded({px+btn_w+12.0f,btn_y,btn_w,36.0f},10.0f,ColorAlpha(UI_ACCENT,0.12f));
        DrawRectRoundedBorder({px+btn_w+12.0f,btn_y,btn_w,36.0f},10.0f,1.5f,ColorAlpha(UI_ACCENT,0.4f));
        DrawTextCentered(ui.font_body,"INSTALLED",
                         px+btn_w+12.0f+btn_w*0.5f,btn_y+11.0f,13.0f,0.5f,UI_ACCENT);
    } else {
        if (UIButtonAccent(BTN_DETAIL_DOWNLOAD,{px+btn_w+12.0f,btn_y,btn_w,36.0f},"DOWNLOAD",13.0f))
...
static void DrawSpinner(float cx,float cy,float radius,float angle) {
    for (int i=0;i<12;i++) {
        ...
        DrawCircleV({cx+cosf(a)*radius,cy+sinf(a)*radius},3.0f+alpha*2.0f,ColorAlpha(UI_ACCENT,alpha));
    }
}
...
void UpdateDrawBeatmaps() {
    ...
    detail_slide.speed=14.0f; detail_slide.Update();
    spinner_angle+=GetFrameTime()*4.0f;

    float wheel=GetMouseWheelMove();
    if (view==BeatmapView::LIST&&wheel!=0.0f) scroll_target-=wheel*80.0f;
    ...
    if (!osu_downloader.HasCredentials()) {
        DrawTextCentered(ui.font_heading,"BEATMAP SEARCH",(float)sw*0.5f,(float)sh*0.5f-20.0f,
                         28.0f,1.0f,ColorAlpha(UI_TEXT_MUTED,0.2f));
        DrawTextCentered(ui.font_body,"Connect your osu! account in Settings to search maps.",
                         (float)sw*0.5f,(float)sh*0.5f+16.0f,14.0f,0.5f,
                         ColorAlpha(UI_TEXT_MUTED,0.15f));
        ui.DrawParticles(); screen_transition.Draw(); return;
    }
    ...
    float filter_btn_h=28.0f, filter_btn_y=bar_y+(bar_h-filter_btn_h)*0.5f;
    ...
    float search_w=fmaxf((float)sw-pad*2.0f-status_total-8.0f-key_total-8.0f-search_btn_w-8.0f, 120.0f);
    ...
    if (hov_search&&IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) search_editing=true;
    if (search_editing&&IsKeyPressed(KEY_ESCAPE)) search_editing=false;

    DrawRectRounded(search_box,8.0f,search_editing?ColorAlpha(UI_ACCENT,0.1f):UI_SURFACE2);
    DrawRectRoundedBorder(search_box,8.0f,1.5f,
        search_editing?UI_ACCENT:(hov_search?ColorLerp(UI_BORDER,UI_ACCENT,0.4f):UI_BORDER));

    if (search_editing) {
        int ch;
        while ((ch=GetCharPressed())!=0) {
            int len=(int)strlen(search_buf);
            if (ch>=32&&len<(int)sizeof(search_buf)-1){search_buf[len]=(char)ch;search_buf[len+1]='\0';}
        }
        if (IsKeyPressed(KEY_BACKSPACE)){int len=(int)strlen(search_buf);if(len>0)search_buf[len-1]='\0';}
        if (IsKeyPressed(KEY_ENTER)||IsKeyPressed(KEY_KP_ENTER)){search_editing=false;StartSearch();}
        if ((IsKeyDown(KEY_LEFT_CONTROL)||IsKeyDown(KEY_RIGHT_CONTROL))&&IsKeyPressed(KEY_V)) {
            const char* clip=GetClipboardText();
            if (clip){int cur=(int)strlen(search_buf),space=(int)sizeof(search_buf)-1-cur;
                if(space>0){strncat(search_buf,clip,space);search_buf[sizeof(search_buf)-1]='\0';}}
        }
    }

    float bm_text_x=search_box.x+10.0f, bm_text_avail=search_box.width-18.0f;
    if (strlen(search_buf)==0&&!search_editing) {
        DrawTextEx(ui.font_body,"Search beatmaps...",{bm_text_x,search_box.y+10.0f},14.0f,0.3f,UI_TEXT_MUTED);
    } else {
        const char* disp=search_buf; int slen=(int)strlen(search_buf);
        for (int i=0;i<slen;i++)
            if (MeasureTextEx(ui.font_body,search_buf+i,14.0f,0.3f).x<=bm_text_avail){disp=search_buf+i;break;}
        BeginScissorMode((int)bm_text_x,(int)search_box.y,(int)bm_text_avail,(int)search_box.height);
        DrawTextEx(ui.font_body,disp,{bm_text_x,search_box.y+10.0f},14.0f,0.3f,UI_TEXT);
        EndScissorMode();
    }
    ...
    for (int i=0;i<4;i++) {
        ...
        if (status_filter==i) UIButtonAccent(BTN_STATUS_ALL+i,fb,status_labels[i],12.0f);
        else if (UIButtonGhost(BTN_STATUS_ALL+i,fb,status_labels[i],12.0f)) {
            status_filter=i;
            if (search_state==SearchState::RESULTS||search_state==SearchState::ERROR) StartSearch();
        }
    ...
    for (int i=0;i<3;i++) {
        ...
        if (keys_filter==i) UIButtonAccent(BTN_KEYS_ANY+i,kb,key_labels[i],12.0f);
    ...
    if (UIButtonAccent(BTN_SEARCH,{search_btn_x,search_btn_y,search_btn_w,search_btn_h},"GO",12.0f)) {
        search_editing=false; StartSearch();
    }
    ...
    float total_list_h=(float)results.size()*(card_h+card_gap)+(has_more?56.0f:0.0f)+list_pad*2.0f;
    ...
    if (search_state==SearchState::IDLE) {
        DrawTextCentered(ui.font_heading,"SEARCH FOR MAPS",list_w*0.5f,(float)sh*0.5f-20.0f,22.0f,1.0f,UI_TEXT_MUTED);
        DrawTextCentered(ui.font_body,"Type in the search box and press GO",list_w*0.5f,(float)sh*0.5f+14.0f,13.0f,0.5f,ColorAlpha(UI_TEXT_MUTED,0.6f));
    } else if (search_state==SearchState::SEARCHING&&results.empty()) {
        DrawSpinner(list_w*0.5f,(float)sh*0.5f,20.0f,spinner_angle);
        DrawTextCentered(ui.font_body,"Searching...",list_w*0.5f,(float)sh*0.5f+36.0f,13.0f,0.5f,UI_TEXT_MUTED);
    } else if (search_state==SearchState::ERROR) {
        DrawTextCentered(ui.font_heading,"SEARCH ERROR",list_w*0.5f,(float)sh*0.5f-20.0f,20.0f,1.0f,UI_DANGER);
        DrawTextCentered(ui.font_body,error_msg.c_str(),list_w*0.5f,(float)sh*0.5f+14.0f,12.0f,0.3f,UI_TEXT_MUTED);
    } else {
        ...
        for (int i=0;i<(int)results.size();i++) {
            ...
            if (cy+card_h>=content_top&&cy<=(float)sh) {
                ...
                int wid=CARD_BASE+(i%(MAX_WIDGET_STATES-CARD_BASE%MAX_WIDGET_STATES));
                if (DrawResultCard(wid,cb,results[i],sel)) {
                    if (selected_idx==i&&view==BeatmapView::DETAIL) {
                        view=BeatmapView::LIST; detail_slide.Set(0.0f);
                        selected_idx=-1; UnloadCover();
                    } else {
                        selected_idx=i; view=BeatmapView::DETAIL; detail_slide.Set(1.0f);
        ...
        if (search_state==SearchState::SEARCHING&&!results.empty())
            DrawSpinner(list_w*0.5f,content_top+list_pad+(float)results.size()*(card_h+card_gap)+20.0f,12.0f,spinner_angle);
        if (has_more&&search_state!=SearchState::SEARCHING) {
            float lmy=content_top+list_pad+(float)results.size()*(card_h+card_gap)+8.0f;
            if (UIButtonGhost(BTN_LOAD_MORE,{list_w*0.5f-80.0f,lmy,160.0f,36.0f},"LOAD MORE",13.0f))
                StartSearch(true);
        }
        if (results.empty()&&search_state==SearchState::RESULTS) {
            DrawTextCentered(ui.font_heading,"NO RESULTS",list_w*0.5f,(float)sh*0.5f-20.0f,22.0f,1.0f,UI_TEXT_MUTED);
            DrawTextCentered(ui.font_body,"Try a different search or filter.",list_w*0.5f,(float)sh*0.5f+14.0f,13.0f,0.5f,ColorAlpha(UI_TEXT_MUTED,0.6f));
        }
    }
    EndScissorMode();

    if (detail_slide.value>0.01f&&selected_idx>=0&&selected_idx<(int)results.size()) {
        float slide=(1.0f-detail_slide.value)*(detail_w+20.0f);
        DrawDetailPanel(list_w+4.0f+slide, content_top+4.0f,
                        detail_w-8.0f, content_h-8.0f,
                        results[selected_idx], s_is_installed);
    }
