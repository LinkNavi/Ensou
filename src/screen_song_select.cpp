...
static Color DiffColor(float stars) {
    if (stars < 2.0f) return Color{100, 200, 255, 255};
    if (stars < 3.5f) return UI_ACCENT;
    if (stars < 5.0f) return Color{255, 200, 80,  255};
    if (stars < 6.5f) return Color{255, 130, 80,  255};
...
static std::string TruncStr(Font font, const std::string& text,
                             float max_w, float size, float spacing) {
    if (MeasureTextEx(font, text.c_str(), size, spacing).x <= max_w) return text;
    std::string t = text;
    while (!t.empty()) {
        t.pop_back();
...
// ─── Draw song card ───────────────────────────────────────────────────────────
static bool DrawSongCard(int id, Rectangle r, const BeatmapGroup& g, bool selected) {
    ...
    DrawRectRounded(rr, 10.0f,
        selected ? UI_SURFACE2 : ColorLerp(UI_SURFACE, UI_SURFACE2, ws.hover.value));
    DrawRectRoundedBorder(rr, 10.0f, 1.5f,
        selected ? ColorAlpha(UI_ACCENT, 0.6f)
    ...
    if (has_tex) {
        ...
        DrawRectangleRoundedLinesEx({cvx, cvy, cv, cv},
            8.0f/cv, 8, 1.0f, ColorAlpha(UI_ACCENT, 0.3f));
    } else {
        ...
        if (!g.title.empty()) {
            ...
            DrawTextCentered(ui.font_heading, letter, cvx+cv*0.5f, cvy+cv*0.5f-13.0f,
                             22.0f, 0.5f, ColorAlpha(UI_ACCENT, 0.85f));
        }
    }
    ...
    DrawTextEx(ui.font_body, artist.c_str(), {tx, rr.y+30.0f}, 12.0f, 0.3f, UI_TEXT_MUTED);

    float rx2 = rr.x + rr.width - 12.0f;
    char star_buf[16];
    snprintf(star_buf, sizeof(star_buf), "%.2f*", g.top_stars);
    DrawTextRight(ui.font_body, star_buf, rx2, rr.y+12.0f, 12.0f, 0.3f, DiffColor(g.top_stars));

    char diff_buf[24];
    snprintf(diff_buf, sizeof(diff_buf), "%d diff%s", (int)g.diffs.size(),
             g.diffs.size()==1?"":"s");
    DrawTextRight(ui.font_body, diff_buf, rx2, rr.y+28.0f, 11.0f, 0.3f, UI_TEXT_MUTED);

    if (!g.diffs.empty()) {
        char bpm_buf[16];
        snprintf(bpm_buf, sizeof(bpm_buf), "%.0f BPM", g.diffs[0].DominantBpm());
        DrawTextRight(ui.font_body, bpm_buf, rx2, rr.y+44.0f, 11.0f, 0.3f, UI_TEXT_MUTED);
    }
...
static void DrawDetailPanel(float x, float y, float w, float h,
    ...
    if (s_cover.id > 0) {
        ...
        DrawRectangleGradientV((int)px, (int)(py + cvh*0.5f),
                               (int)cvw, (int)(cvh*0.5f),
                               ColorAlpha(BLACK, 0), ColorAlpha(BLACK, 0.55f));
        DrawRectangleRoundedLinesEx({px, py, cvw, cvh},
            8.0f/fminf(cvw,cvh), 8, 1.5f, ColorAlpha(UI_ACCENT, 0.25f));
    } else {
        ...
        if (!g.title.empty()) {
            char letter[2] = {(char)toupper((unsigned char)g.title[0]), '\0'};
            DrawTextCentered(ui.font_heading, letter, px+cvw*0.5f, py+cvh*0.5f-22.0f,
                             44.0f, 0.5f, ColorAlpha(UI_ACCENT, 0.4f));
        }
    }
    py += cvh + 14.0f;
    ...
    py += 22.0f;
    DrawTextEx(ui.font_body, g.artist.c_str(), {px, py}, 12.0f, 0.3f, UI_TEXT_MUTED);
    py += 16.0f;
    ...
    for (int i = 0; i < (int)g.diffs.size() && shown < max_show; i++, shown++) {
        ...
        if (hov && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) sel_diff = i;

        DrawRectRounded(dr, 5.0f,
            sel  ? ColorAlpha(UI_ACCENT, 0.15f) :
            hov  ? ColorAlpha(UI_SURFACE2, 0.8f) : Color{});
        if (sel) DrawRectRoundedBorder(dr, 5.0f, 1.0f, ColorAlpha(UI_ACCENT, 0.4f));
        ...
        DrawTextEx(ui.font_body, vname.c_str(), {px+20.0f, py+dh2*0.5f-7.0f},
                   12.0f, 0.3f, sel ? UI_ACCENT : UI_TEXT);

        char sb[16];
        snprintf(sb, sizeof(sb), "%.2f*", g.diffs[i].difficulty.star_rating);
        DrawTextRight(ui.font_body, sb, x+w-18.0f, py+dh2*0.5f-7.0f, 11.0f, 0.3f, dc);
        py += dh2 + dgap;
    }
    ...
    if (sel_diff < (int)g.diffs.size()) {
        ...
        char bpm_buf[24];
        snprintf(bpm_buf, sizeof(bpm_buf), "%.0f BPM", g.diffs[sel_diff].DominantBpm());
        DrawTextEx(ui.font_body, len_str.c_str(), {px, iy}, 12.0f, 0.3f, UI_TEXT_MUTED);
        DrawTextRight(ui.font_body, bpm_buf, x+w-18.0f, iy, 12.0f, 0.3f, UI_TEXT_MUTED);
    }

    float btn_y = y + h - 48.0f;
    if (UIButtonAccent(SS_BTN_PLAY, {px, btn_y, w-36.0f, 36.0f}, "PLAY", 14.0f)) {
...
void UpdateDrawSongSelect() {
    ...
    if (hov_s && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) s_search_editing = true;
    if (s_search_editing && IsKeyPressed(KEY_ESCAPE))     s_search_editing = false;
    ...
    if (s_search_editing) {
        ...
        while ((ch = GetCharPressed()) != 0) {
            ...
            if (ch >= 32 && len < (int)sizeof(s_search)-1)
                { s_search[len] = (char)ch; s_search[len+1] = '\0'; }
        }
        if (IsKeyPressed(KEY_BACKSPACE)) {
            int len = (int)strlen(s_search);
            if (len > 0) s_search[len-1] = '\0';
        }
        if ((IsKeyDown(KEY_LEFT_CONTROL)||IsKeyDown(KEY_RIGHT_CONTROL)) && IsKeyPressed(KEY_V)) {
            const char* clip = GetClipboardText();
            if (clip) {
                int cur = (int)strlen(s_search), space = (int)sizeof(s_search)-1-cur;
                if (space > 0) { strncat(s_search, clip, space); s_search[sizeof(s_search)-1]='\0'; }
            }
        }
    }
    ...
    if (empty_search && !s_search_editing) {
        DrawTextEx(ui.font_body, "Search songs...",
                   {text_x, search_box.y + 10.0f}, 14.0f, 0.3f, UI_TEXT_MUTED);
    } else {
        const char* disp = s_search;
        int slen = (int)strlen(s_search);
        for (int i = 0; i < slen; i++) {
            if (MeasureTextEx(ui.font_body, s_search+i, 14.0f, 0.3f).x <= text_avail)
                { disp = s_search+i; break; }
        }
        BeginScissorMode((int)text_x, (int)search_box.y, (int)text_avail, (int)search_box.height);
        DrawTextEx(ui.font_body, disp, {text_x, search_box.y+10.0f}, 14.0f, 0.3f, UI_TEXT);
        EndScissorMode();
    }
    ...
    float total_h   = (float)s_filtered.size() * (card_h + card_gap) + list_pad * 2.0f;
    float max_scroll = fmaxf(0.0f, total_h - content_h);

    float wheel = GetMouseWheelMove();
    if (wheel != 0.0f) s_scroll_target -= wheel * 80.0f;
    ...
    if (s_filtered.empty()) {
        ...
        DrawTextCentered(ui.font_heading, msg,
                         list_w*0.5f, content_top+content_h*0.5f-16.0f,
                         20.0f, 1.0f, UI_TEXT_MUTED);
    ...
    for (int i = 0; i < (int)s_filtered.size(); i++) {
        ...
        if (cy + card_h >= content_top && cy <= (float)sh) {
            ...
            int wid  = SS_CARD_BASE + (i % (MAX_WIDGET_STATES - SS_CARD_BASE % MAX_WIDGET_STATES));
