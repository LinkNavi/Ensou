  ...
  for (const std::string &folder : folders) {
    ...
    for (const std::string &file : files) {
      ...
      if (group.title.empty()) {
        group.title =
            meta.title_unicode.empty() ? meta.title : meta.title_unicode;
        group.artist =
            meta.artist_unicode.empty() ? meta.artist : meta.artist_unicode;
...
int BeatmapLibrary::Rescan() { return Scan(songs_dir); }

// ─── BeatmapLibrary::LoadDiff
// ─────────────────────────────────────────────────

bool BeatmapLibrary::LoadDiff(BeatmapGroup &group, int diff_index,
