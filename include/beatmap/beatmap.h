...
// ─── Note types
// ─────────────────────────────────────────────────────────────── Standard
// osu!mania types are covered by NORMAL and HOLD. Custom types start at
// CUSTOM_0 — the .ensou format can use any of these.
enum class NoteType : uint8_t {
  ...
  SWAP = 64, // Mirrors it's position once it reaches the middle of the screen
  MINE = 65, // Counts as a miss if you hit
REBOUND = 66, // Bounce's back a set number of beats on hit to be hit again can be repeated
...
struct TimingPoint {
  ...
  float sv_multiplier; // Scroll velocity multiplier (inherited points,
                       // default 1.0)
...
// ─── Beatmap
// ────────────────────────────────────────────────────────────────── One
// Beatmap = one difficulty of one song. Multiple difficulties for the same song
// are separate Beatmap objects grouped by their shared audio file / folder.
struct Beatmap {
  // ── Identity ──────────────────────────────────────────────────────────────
  ...
  float DominantBpm() const {
    ...
    for (int i = 0; i < (int)timing_points.size(); i++) {
      if (timing_points[i].inherited)
        continue;
      int end = (i + 1 < (int)timing_points.size())
                    ? timing_points[i + 1].time_ms
                    : (notes.empty() ? timing_points[i].time_ms
                                     : notes.back().time_ms);
